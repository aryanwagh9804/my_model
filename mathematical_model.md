...
*   **Outputs ($O$)**: The set of operational endpoints.
    $$O = \{ \text{ID}_{identified}, \text{Action}_{motor}, \text{Audio}_{out}, Database_{log} \}$$
*   **Processes ($P$)**: The mapping functions that transform $I \to O$.
    $$P = \{ F_{detect}, F_{liveness}, F_{recognize}, F_{antispoof} \}$$
...
$$A_u = \begin{cases} 1 & \text{if } F_{detect}(f_t) \land F_{liveness}(f_t) \land (F_{recognize}(f_t)=u) \land \neg F_{antispoof}(f_t) \\\\ 0 & \text{otherwise} \end{cases}$$
...
$$f(x) = \sum_{(x,y) \in R_{white}} i(x,y) \ - \sum_{(x,y) \in R_{dark}} i(x,y)$$
...
To prevent photo-spoofing, the system maps 6 facial landmarks (points $p_1, p_2, p_3, p_4, p_5, p_6$) on the perimeter of the user's eye and computes their geometric distances.
...
*   **Mathematical Condition for Liveness**: A blink is confirmed if $EAR < \tau_{blink}$ (where the threshold $\tau_{blink} \approx 0.2$) for a frame, indicating the vertical distance went to $0$.
...
To find who the person is, the system calculates the **Chi-Square distance** between the camera's generated histogram $H_{cam}$ and a saved database histogram $H_{db}$:
$$D(H_{cam}, H_{db}) = \sum_{i=1}^{n} \frac{(H_{cam}(i) - H_{db}(i))^2}{H_{cam}(i) + H_{db}(i)}$$
*   The ID with the minimum distance $D_{min}$ is the predicted identity. A lower numerical distance is mathematically interpreted as a stronger visual match.
...
$$Confidence = Pr(Object) \times \text{IoU}_{truth}^{pred}$$
where $\text{IoU}$ stands for *Intersection over Union* (Area of Overlap divided by Area of Union).

**Class-specific Safety Condition**:
$$CS_i = Pr(\text{Class}_i | Object) \times Confidence$$
If any bounding box possesses $CS_{phone} > \tau_{phone}$, the system logically triggers $F_{antispoof}(f_t) = \text{True}$. The core constraint (from Section 1) instantly fails, refusing to mark attendance.
